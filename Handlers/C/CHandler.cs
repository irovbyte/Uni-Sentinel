...
internal sealed partial class CHandler : BaseHandler
{
    ...
    public override async Task<(bool Ok, int Points)> CheckMemoryAsync() => (await ValgrindAnalyzer.CheckAsync(ProjectPath, RunAsync), 0);
    public override async Task<(bool Ok, int Points)> CheckCpuAsync() => (await CoverageAnalyzer.AnalyzeAsync(ProjectPath, RunAsync), 0);
